...
let links_menu = document.querySelectorAll('.menu-items');
let linkImages = document.querySelectorAll('.hover-reveal_img');

links_menu.forEach((link_menu,idx) => {
    link_menu.addEventListener('mouseover', () =>
    {
        innerCursor.classList.add('grow');
    });

    link_menu.addEventListener('mouseleave', ()=>
    {
        innerCursor.classList.remove('grow');
    });
})
